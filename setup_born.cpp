...
void setup_born(){
  ...
  for(int i = 0; i < txproc; i++)
    ...
    for(int n = 0; n < numunk; n++)
      ...
      //inc[i*numunk+n]=complex<double>(0,0.25)*hn(0,k0*abs(pos[n]-tx[mytx+i]));
    /*{inc[i*numunk+n]=0;
      for(int m = 0; m < numrx; m++){
  ...
  for(int n = 0; n < numunk; n++){
    ...
    if(abs(pos[n]-complex<double>(-9.6,0))<1.5)
        o[n] = eps*cos(abs(pos[n]-complex<double>(-9.6,0))/(1.5/M_PI*2));
    if(abs(pos[n]-complex<double>(-3.2,0))<1)
        o[n] = eps*cos(abs(pos[n]-complex<double>(-3.2,0))/(1/M_PI*2));
    if(abs(pos[n]-complex<double>(+3.2,0))<0.5)
        o[n] = eps*cos(abs(pos[n]-complex<double>(3.2,0))/(0.5/M_PI*2));
    if(abs(pos[n]-complex<double>(+9.6,0))<0.25)
        o[n] = eps*cos(abs(pos[n]-complex<double>(9.6,0))/(0.25/M_PI*2));*/
    //if(abs(pos[n]-complex<double>(0,6))<3)
    //    o[n] = eps*cos(abs(pos[n]-complex<double>(0,6))/(3/M_PI*2));
    //if(abs(pos[n])<10)o[n]=0.1;
    //if(abs(pos[n])<8)
    ...
    if(pos[n].real()<8 && pos[n].real()>-8 && pos[n].imag()<8 && pos[n].imag()>-8)
      ...
      if(abs(pos[n])<3)
        o[n] = 0.01;
    //if(pos[n].real()>5 && pos[n].real()<-5 && pos[n].imag()>5 && pos[n].imag()<-5)
    //if(abs(pos[n])<res)
    //  o[n] = 1;
  }
  /*if(myid==0){
    printf("READ FROM FILE\n");
    FILE *inputf = fopen("../phantom_512.txt","r");
    for(int n = 0; n < numunk; n++){
      float val = 0;
      fscanf(inputf,"%f\n",&val);
  ...
  for(int i = 0; i < txproc; i++){
    ...
    for(int n = 0; n < numunk; n++)
      buff[n] = c[unkmap[n]];
    sprintf(strtemp,"sca_%d_of%d.bin",i,myid_dbim);
    FILE *scaf = fopen(strtemp,"wb");
    fwrite(buff,sizeof(complex<double>),numunk,scaf);
    fclose(scaf);*/
  }
  ...
  MPI_Allreduce(&errtot,&mesnorm,1,MPI_DOUBLE,MPI_SUM,MPI_COMM_DBIM);
}
...
complex<double> sign(complex<double> o){
  complex<double> on(-1,-1);
  if(o.real() > 0) on.real(1);
  if(o.real() < 0) on.real(-1);
  if(o.imag() > 0) on.imag(1);
  if(o.imag() < 0) on.imag(-1);
  return on;
}
