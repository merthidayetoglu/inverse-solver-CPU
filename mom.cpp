...
int main(int argc, char** argv){
  ...
  for(int m = 0; m < numtx; m++){
    double xpos = -25.6+m*51.2/(numtx-1);
    tx[m] = complex<double>(xpos,15);
  }
  rx = new complex<double>[numrx];
  for(int m = 0; m < numrx; m++){
    double xpos = -25.6+m*51.2/(numrx-1);
    rx[m] = complex<double>(xpos,15);
  }*/
  ...
  for(int i = 0; i < txproc; i++){
    ...
    #pragma omp parallel for
    for(int n = 0; n < amount; n++)
      buff[addres+n] = real(buff[addres+n])+l1l2*regular1*sign(o[addres+n])+(1-l1l2)*regular2*regular2*(o[addres+n]);
      //buff[addres+n] = buff[addres+n]+l1l2*regular1*sign(o[addres+n])+(1-l1l2)*regular2*regular2*(o[addres+n]);
  }
